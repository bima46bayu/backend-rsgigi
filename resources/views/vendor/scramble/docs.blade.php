...
<head>
    ...
    <script>
        ...
        // Intercept TryIt requests to inject tokens and CSRF cookies automatically
        window.fetch = async (url, options) => {
            ...
            // Setup options and headers object
            const fetchOptions = { ...options };
            if (!fetchOptions.headers) {
                fetchOptions.headers = new Headers();
            }

            // Inject CSRF token if exists
            const csrfToken = getCookieValue(CSRF_TOKEN_COOKIE_KEY);
            if (csrfToken) {
                updateFetchHeaders(fetchOptions.headers, CSRF_TOKEN_HEADER_KEY, decodeURIComponent(csrfToken));
            }

            const urlString = url.toString();
            const isLoginRequest = urlString.includes('/login');
            const isLogoutRequest = urlString.includes('/logout');

            // Inject stored token automatically for all requests except login
            const storedToken = localStorage.getItem('scramble_token');
            if (storedToken && !isLoginRequest) {
                updateFetchHeaders(fetchOptions.headers, 'Authorization', `Bearer ${storedToken}`);
            }

            try {
                const response = await originalFetch(url, fetchOptions);

                // If logging out successfully, clear the token
                if (isLogoutRequest && response.status === 200) {
                    localStorage.removeItem('scramble_token');
                    console.log('[Scramble Auth] Token cleared on logout.');
                }

                // If logging in successfully, capture and store the token automatically
                if (isLoginRequest && response.status === 200) {
                    try {
                        ...
                        if (data.token) {
                            ...
                            console.log('[Scramble Auth] Token automatically saved from login response.');
                        }
                    } catch (e) {
                        console.error('[Scramble Auth] Failed to parse token from login response', e);
                    }
                }
